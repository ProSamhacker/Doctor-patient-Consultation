...
interface PatientDao {
    ...
    @Query("SELECT * FROM patients WHERE patientId = :id") suspend fun getById(id: String): Patient?

    @Query("SELECT * FROM patients") fun getAll(): Flow<List<Patient>>

    @Query(
            "SELECT * FROM patients WHERE name LIKE '%' || :query || '%' OR phone LIKE '%' || :query || '%'"
    )
