...
@Dao
interface DoctorDao {
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insert(doctor: Doctor)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertDoctors(doctors: List<Doctor>)

    @Update suspend fun update(doctor: Doctor)

    @Delete suspend fun delete(doctor: Doctor)

    @Query("SELECT * FROM doctors WHERE doctorId = :id") suspend fun getById(id: String): Doctor?

    @Query("SELECT * FROM doctors WHERE isActive = 1") fun getAllActive(): Flow<List<Doctor>>
    ...
    @Query(
            "SELECT * FROM doctors WHERE name LIKE '%' || :query || '%' OR specialization LIKE '%' || :query || '%'"
    )
