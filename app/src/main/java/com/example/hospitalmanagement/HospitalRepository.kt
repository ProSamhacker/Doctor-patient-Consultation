...
class HospitalRepository(
        ...
        fun getUnreadMessageCount(appId: Int, userId: String) = messageDao.getUnreadCount(appId, userId)
        ...
        suspend fun startConsultation(appId: Int): Long {
                val session = ConsultationSession(appId = appId, isRecording = true, startTime = System.currentTimeMillis())
        ...
        suspend fun endConsultation(sessionId: Int, transcript: String) {
                ...
                session?.let {
                        ...
                        consultationSessionDao.update(it.copy(isRecording = false, endTime = System.currentTimeMillis(), duration = duration, fullTranscript = transcript))
                }
        }
        ...
        // Old method for medical terms (kept for backward compatibility, but getGeneralAiResponse is better)
